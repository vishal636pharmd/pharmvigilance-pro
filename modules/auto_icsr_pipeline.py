...
YOUR_EMAIL = os.environ.get("SMTP_FROM_EMAIL", "").strip()
YOUR_PASSWORD = os.environ.get("SMTP_APP_PASSWORD", "").replace(" ", "")

# PvPI official email for ADR submission (legitimate alternate channel)
PVPI_EMAIL = os.environ.get("PVPI_RECIPIENT_EMAIL", "pvpi@ipcindia.net").strip()
SMTP_HOST = os.environ.get("SMTP_HOST", "smtp.gmail.com").strip()
SMTP_PORT = int(os.environ.get("SMTP_PORT", "465"))
COPY_TO_EMAIL = os.environ.get("SUBMISSION_COPY_TO", YOUR_EMAIL).strip()
# ─────────────────────────────────────────────────────────────────────────────
...
def run_full_pipeline(report_id):
    ...
    if YOUR_EMAIL and YOUR_PASSWORD and result["icsr_generated"]:
        ...
        result["steps"].append({
            ...
            "recipients": [PVPI_EMAIL, COPY_TO_EMAIL]
        })
    else:
        result["steps"].append({
            "step": "6_email",
    ...
    result["message"] = (
        ...
        f"({naranjo_result['category']}). "
        f"Report emailed to PvPI for review at {PVPI_EMAIL}."
    )
    return result
...
def _send_icsr_email(drug_name, meddra_pt, report_id,
    ...
    try:
        msg = MIMEMultipart()
        msg["From"]    = YOUR_EMAIL
        recipients = [PVPI_EMAIL]
        if COPY_TO_EMAIL and COPY_TO_EMAIL not in recipients:
            recipients.append(COPY_TO_EMAIL)
...
Alternative submission: submit to the configured PvPI or AMC address, or call 1800-180-3024.
...
"""
        ...
        with smtplib.SMTP_SSL(SMTP_HOST, SMTP_PORT, timeout=30) as server:
            ...
            if refused:
                raise RuntimeError(f"Recipient refused by mail server: {refused}")

        print(f"[ICSR Email] Sent for PvPI review to {', '.join(recipients)}")
        return True

    except Exception as e:
        print(f"[ICSR Email] Delivery failed: {type(e).__name__}: {e}")
        print("[ICSR Email] Check SMTP settings, the App Password, and recipient address.")
        return False
...
def _update_icsr_queue(report_id, drug_name, meddra_pt,
    ...
    conn = get_conn()
    c = conn.cursor()
    values = (drug_name, meddra_pt, check_result, status,
              pdf_path, xml_path, str(datetime.now()), report_id)
    c.execute(q("SELECT queue_id FROM icsr_queue WHERE report_id=?"), (report_id,))
    if c.fetchone():
        c.execute(q("""
            UPDATE icsr_queue SET drug_name=?, meddra_pt=?, check_result=?,
            icsr_status=?, pdf_path=?, xml_path=?, created_at=?
            WHERE report_id=?
        """), values)
    else:
        c.execute(q("""
            INSERT INTO icsr_queue
