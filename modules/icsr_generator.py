    ...
    try:
        c.execute(q("""
            SELECT sr.report_id, sr.drug_name, sr.raw_symptom,
                   ...
                   p.bill_date,
                   (
                       SELECT aa.naranjo_score
                       FROM adr_assessments aa
                       WHERE aa.patient_id = sr.patient_id
                         AND aa.drug_name = sr.drug_name
                         AND (aa.meddra_pt = sr.meddra_pt
                              OR aa.meddra_pt IS NULL
                              OR sr.meddra_pt IS NULL)
                       ORDER BY aa.assessment_id DESC
                       LIMIT 1
                   ) AS naranjo_score,
                   (
                       SELECT aa.naranjo_category
                       FROM adr_assessments aa
                       WHERE aa.patient_id = sr.patient_id
                         AND aa.drug_name = sr.drug_name
                         AND (aa.meddra_pt = sr.meddra_pt
...
def generate_cioms_pdf(report_id):
    ...
    (report_id, drug_name, raw_symptom, meddra_pt, meddra_soc, severity,
     ...
     full_name, age, gender, phone, bill_date, naranjo_score,
     naranjo_category) = row

    naranjo_result = (
        f"{naranjo_score} - {naranjo_category}"
        if naranjo_score is not None and naranjo_category else "Not assessed"
    )
    ...
    adr_data = [
        ...
        ["Naranjo causality assessment", naranjo_result],
    ]
