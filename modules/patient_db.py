...
from modules.db_config import get_connection, get_placeholder, adapt_query, USE_POSTGRES


def _ph():
    """Returns SQL placeholder — %s for PostgreSQL, ? for SQLite."""
...
def init_db():
    ...
    c.execute(f"""CREATE TABLE IF NOT EXISTS patient_profile (
        patient_id {'SERIAL' if USE_POSTGRES else 'INTEGER'} PRIMARY KEY {'DEFAULT' if USE_POSTGRES else 'AUTOINCREMENT'.replace('AUTOINCREMENT','') if not USE_POSTGRES else ''},
        ...
        "SERIAL PRIMARY KEY" if USE_POSTGRES else "INTEGER PRIMARY KEY AUTOINCREMENT"
    ))

    # Simpler approach — use compatible SQL
    tables = [
        ...
        """CREATE TABLE IF NOT EXISTS icsr_queue (
            ...
            pdf_path TEXT, xml_path TEXT, created_at TEXT, submitted_at TEXT
        )""" if USE_POSTGRES else
        """CREATE TABLE IF NOT EXISTS icsr_queue (
    ...
    conn.close()
    _seed_adr_kb()
    print(f"[DB] Database ready. Mode: {'PostgreSQL' if USE_POSTGRES else 'SQLite'}")
...
def _seed_adr_kb():
    ...
    sql = adapt_query(
        "INSERT INTO adr_knowledge_base "
    ...
    conn.commit()
    conn.close()
    print(f"[DB] ADR knowledge base seeded.")
...
def create_or_get_profile(full_name, age, gender, phone):
    ...
    c = conn.cursor()
    ph = _ph()
    c.execute(adapt_query("SELECT patient_id FROM patient_profile WHERE phone=?"), (phone,))
    row = c.fetchone()
    if row:
        pid = row[0]
    else:
...
def get_profile_by_id(patient_id):
    conn = get_connection()
    c = conn.cursor()
    c.execute(adapt_query("SELECT * FROM patient_profile WHERE patient_id=?"), (patient_id,))
    row = c.fetchone()
    conn.close()
    if not row:
        return {}
...
def get_purchase_history(patient_id):
    ...
    c.execute(adapt_query("""
        SELECT p.purchase_id,p.bill_date,p.invoice_no,p.source,
               pd.drug_name,pd.quantity,pd.mrp,pd.drug_id,
               pd.reminder_date,pd.reminder_sent
        FROM purchases p JOIN purchase_drugs pd
...
def get_all_reminders_for_patient(patient_id):
    ...
    c = conn.cursor()
    c.execute(adapt_query(
        "SELECT drug_id,drug_name,reminder_date,reminder_sent "
...
def get_due_reminders():
    ...
    c.execute(adapt_query("""
        SELECT pd.drug_id,pd.patient_id,pd.drug_name,pd.reminder_date,
               pp.full_name,pp.phone
        FROM purchase_drugs pd
        JOIN patient_profile pp ON pd.patient_id=pp.patient_id
        WHERE pd.reminder_date<=? AND pd.reminder_sent=0
    """), (today,))
    rows = c.fetchall()
    conn.close()
...
def mark_reminder_sent(drug_id):
    ...
    c = conn.cursor()
    c.execute(adapt_query(
        "UPDATE purchase_drugs SET reminder_sent=1 WHERE drug_id=?"
    ), (drug_id,))
    conn.commit()
    conn.close()
...
def set_reminder_date_to_today(drug_id):
    ...
    c = conn.cursor()
    c.execute(adapt_query(
        "UPDATE purchase_drugs SET reminder_date=?,reminder_sent=0 WHERE drug_id=?"
...
def save_symptom_report(patient_id, drug_id, drug_name, raw_symptom,
    ...
    if USE_POSTGRES:
        c.execute(sql + " RETURNING report_id", vals)
...
def get_symptom_reports_for_patient(patient_id):
    ...
    c.execute(adapt_query("""
        ...
        FROM symptom_reports WHERE patient_id=?
...
def update_meddra_term(report_id, meddra_pt, meddra_soc):
    ...
    c = conn.cursor()
    c.execute(adapt_query(
        "UPDATE symptom_reports SET meddra_pt=?,meddra_soc=? WHERE report_id=?"
...
def save_adr_assessment(patient_id, drug_name, meddra_pt,
    ...
    if USE_POSTGRES:
        c.execute(sql + " RETURNING assessment_id", vals)
        aid = c.fetchone()[0]
    else:
        c.execute(sql, vals)
        aid = c.lastrowid
...
def get_all_assessments():
    ...
    try:
        c.execute("""
            SELECT a.assessment_id,pp.full_name,a.drug_name,
...
def update_assessment_status(assessment_id, status):
    ...
    c = conn.cursor()
    c.execute(adapt_query(
        "UPDATE adr_assessments SET pharmacist_status=?,review_date=? "
        "WHERE assessment_id=?"
